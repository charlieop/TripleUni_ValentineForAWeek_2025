...
class MatchedPair(models.Model):
    pair_number = models.AutoField(primary_key=True, editable=False, verbose_name="组号")
    ...
    applicant1 = models.ForeignKey(
        "Applicant",
        on_delete=models.PROTECT,
        related_name="matched_pairs1",
        verbose_name="嘉宾1号"
    )
    applicant2 = models.ForeignKey(
        "Applicant",
        on_delete=models.PROTECT,
        related_name="matched_pairs2",
        verbose_name="嘉宾2号"
    )
    ...
    def __str__(self):
        return f"#{self.pair_number}-{self.name}  | Mentor: {self.mentor.name}"
...
class Task(models.Model):
    ...
    def __str__(self):
        return f"第{self.day}天: #{self.pair.pair_number}-{self.pair.name}"
...
class Image(models.Model):
    def generateUploadPath(self, filename):
        ...
        return f"uploads/{self.task.pair.pair_number}/day-{self.task.day}/{modified_filename}"
